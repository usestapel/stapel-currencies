"""Single-module Django settings for stapel-currencies' contract harness.
...
emission (``_codegen.py`` / ``make contract``): a ``{currencies + core}``
instance mounted at the canonical ``/currencies/api/v1/`` prefix
(api-versioning.md §2) with the *production* ``REST_FRAMEWORK`` (the
canonical stapel-core config, inlined as dotted paths — importing it at
configure() time would freeze DRF defaults), so the emitted
``schema.json`` / ``flows.json`` paths are byte-identical to a monolith
aggregate's currencies slice (contract-pipeline.md §2).
"""
from __future__ import annotations


def settings_kwargs(*, root_urlconf: str = "stapel_currencies.codegen_urls") -> dict:
    ...
    # settings read (contract-pipeline.md §3).
    rest_framework = {
    ...
    return dict(
        ...
        MIGRATION_MODULES={"users": None, "currencies": None},
        REST_FRAMEWORK=rest_framework,
    )


# drf-spectacular derives the operationId prefix from the common path of all
# endpoints — "/" across a multi-module monolith, but "/currencies/api/v1" in
# a single-module harness (which would strip the mount from operationIds).
# Pin it to the monolith's common prefix so operationIds match the aggregate
